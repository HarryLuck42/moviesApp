...
import kotlinx.android.synthetic.main.movie_upcoming_fragment.view.*
...
class MoviesUpcomingFragment : Fragment() {
    ...
    companion object {
        fun newInstance() = MoviesUpcomingFragment()
    }
    ...
    override fun onCreateView(
        ...
        val v = inflater.inflate(R.layout.movie_upcoming_fragment, container, false)
        ...
        v.rv_movies_up.layoutManager = layoutManager
        v.rv_movies_up.setHasFixedSize(true)
        v.rv_movies_up.isFocusable = false
        v.rv_movies_up.visibility = View.VISIBLE
        v.tv_not_found_movie_up.visibility = View.GONE
        v.iv_not_found_up.visibility = View.GONE
        adapter = MovieAdapter(
            this.requireContext(),
            upViewModel.listMovie, MovieListener{ id ->
                upViewModel.onMovieClicked(id)
            }
        )
        adapter.notifyDataSetChanged()
        v.rv_movies_up.adapter = adapter
        upViewModel.navigateToDetail.observe(viewLifecycleOwner, Observer {
            it?.let {
                this.findNavController().navigate(MoviesUpcomingFragmentDirections.actionMovieUpcomingFragmentToDetailMovieFragment(it))
        ...
        upViewModel.getListData(
            Const.apikey, paging.toString(), Const.language,
            upViewModel.genres.value, upViewModel.fisrt_date.value, upViewModel.last_date.value,
            upViewModel.min_vote.value, upViewModel.max_vote.value)
        return v
    }
    ...
    private fun RefreshListMovie() {
        ...
        upViewModel.resetGenre()
        upViewModel.resetFirstDate()
        upViewModel.resetLastDate()
        upViewModel.resetMinVote()
        upViewModel.resetMaxVote()
        upViewModel.inputSearchState(false)
        upViewModel.getListData(
            Const.apikey, paging.toString(), Const.language,
            upViewModel.genres.value, upViewModel.fisrt_date.value, upViewModel.last_date.value,
    ...
    private fun initObserver(v : View){
        upViewModel.movieState.observe(viewLifecycleOwner, Observer {
            when (it){
                is UiState.Loading -> {
                ...
                is UiState.Success -> {
                    ...
                    if(tempPagging==1){
                        if (it.data.results!!.isEmpty()){
                            v.rv_movies_up.visibility = View.GONE
                            v.iv_not_found_up.visibility = View.VISIBLE
                            v.tv_not_found_movie_up.visibility = View.VISIBLE
                        }else{
                            v.rv_movies_up.visibility = View.VISIBLE
                            v.iv_not_found_up.visibility = View.GONE
                            v.tv_not_found_movie_up.visibility = View.GONE
    ...
    private fun initObserverSearchMovie(v : View){
        upViewModel.searchMovieState.observe(viewLifecycleOwner, Observer {
            when (it){
                is UiState.Loading -> {
    ...
    private fun onScrollAdapter(v : View){
        ...
        v.rv_movies_up.addOnScrollListener(object : RecyclerView.OnScrollListener(){
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)
                if(dy > 0){

                    val lastVisibleItem = (v.rv_movies_up.layoutManager!! as GridLayoutManager).findLastVisibleItemPosition()
                    val totalItemCount = (v.rv_movies_up.layoutManager!! as GridLayoutManager).itemCount
                    if (upViewModel.isLoading && totalItemCount <= (lastVisibleItem + 1)) {
                        upViewModel.stopLoading()
                        if(upViewModel.isSearchState.value!!){
                            upViewModel.searchMovieByKeyword(Const.apikey, paging.toString(), Const.language,
                                upViewModel.keywordValue.value)
                        }else{
                            upViewModel.getListData(Const.apikey, paging.toString(), Const.language,
                                upViewModel.genres.value, upViewModel.fisrt_date.value, upViewModel.last_date.value,
                                upViewModel.min_vote.value, upViewModel.max_vote.value)
                        }
                        progressDialog.show()
    ...
    private fun settingDialogSort(v : View){
        ...
        upViewModel.getAllTvSeries!!.observe(viewLifecycleOwner, Observer {
...
//                    viewModel.onMovieClicked(id)
        ...
        customview.btn_submit_sort.setOnClickListener {
            ...
            if(genresResult.isEmpty()){
                ...
                upViewModel.inputGenres(genresResult)
            }
            ...
            if(startDate.isEmpty() && endDate.isEmpty()){
                ...
                upViewModel.inputFirstDate(startDate)
                upViewModel.inputLastDate(endDate)
            }
            ...
            if(minVote >= maxVote){
                ...
                upViewModel.inputMinVote(minVoteResult)
                upViewModel.inputMaxVote(maxVoteResult)
            }
