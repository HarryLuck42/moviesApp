...
class MoviesPopularViewModel @ViewModelInject constructor(val moviesRepository: MoviesRepository,
    ...
    val genreState = MutableLiveData<UiState<GenreResponse>>()
    ...
    fun saveGenreMasterData(api_key: String, language:String){
        genreState.value = UiState.Loading()

        scope.launch {
            try {
                val result = moviesRepository.getDataGenres(api_key, language).await()
                moviesRepository.replaceAllGenre(result.genres!!)
                genreState.postValue(UiState.Success(result))

            }catch (e: Exception){
                genreState.postValue(UiState.Error(e))
            }
        }
    }
